...
public class Pawn : Monster
{
    ...
    public IEnumerator MoveAni()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                float dist = Vector3.SqrMagnitude(target.position - transform.position + Quaternion.Euler(0, 90 * i, 0) * Vector3.forward);
                if (dist < minDist)
                {
                    minDist = dist;
                    dir = i;
        ...
        if (dir != -1)
        {
            ...
            for (float i = 0; i < 1; i += Time.deltaTime * jumpSpeed)
            {
                transform.position = new Vector3(originPos.x, Mathf.Sin(Mathf.PI * i) * 1.5f, originPos.z) + Quaternion.Euler(0, 90 * i, 0) * Vector3.forward * i;
                yield return null;
            }
        }
    }
