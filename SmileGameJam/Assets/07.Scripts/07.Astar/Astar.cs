...
public class Astar : MonoBehaviour
{
    ...
    public Vector3[] FindPath(Vector3 pathStart, Vector3 pathEnd)
    {
        ...
        if (startNode.walkable && targetNode.walkable)
        {
            ...
            if (!pathSuccess)
            {
                Node newTargetNode = currentNode;
                foreach (Node n in closedSet)
                {
                    if (startNode == n)
                        continue;

                    if (n.hCost < newTargetNode.hCost)
                        newTargetNode = n;
                }
                return FindPath(pathStart, newTargetNode.worldPosition);
            }
        }
